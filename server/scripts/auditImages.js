...
async function auditImages() {
  ...
  const classifyUrl = (url) => {
    ...
    if (url.startsWith("http://") || url.startsWith("https://"))
      return "⚠️  External URL: " + new URL(url).hostname;
  ...
  const { data: events, error: evError } = await supabase
    .from("events")
    .select("id, title, img, images")
    .limit(50);
  if (evError) {
    console.error("  Error:", evError.message);
  } else {
    events.forEach((ev) => {
  ...
  const { data: gallery, error: galError } = await supabase
    ...
    .limit(10);
  if (galError) {
    console.error("  Error:", galError.message);
  } else {
  ...
  const { data: programs, error: progError } = await supabase
    .from("programs")
    .select("id, title, image, thumbnails")
    .limit(20);
  if (progError) {
    ...
    programs.forEach((prog) => {
      ...
      if (prog.thumbnails) {
        ...
        thumbs
          .slice(0, 2)
          .forEach((t, i) => console.log(`    thumbnails[${i}]: ${classifyUrl(t)}`));
      }
    });
  }
