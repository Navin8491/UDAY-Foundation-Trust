...
export function Donations() {
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute(
      "download",
      `uday_donations_export_${new Date().toISOString().split("T")[0]}.csv`,
    );
        ...
        {[
          {
            ...
            value: `₹${todayDonations.toLocaleString("en-IN")}`,
            desc: "Today's success",
            color: "text-[#7A9D1C] bg-[#7A9D1C]/10",
          },
          {
            title: "Monthly Donations",
            value: `₹${monthlyDonations.toLocaleString("en-IN")}`,
            desc: "This Month",
              ...
              {loading ? (
                ...
                filtered.map((d) => (
                  <tr key={d.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="py-4 px-6 font-mono text-slate-900 select-all">
                      {d.id.substring(0, 8)}...
                    </td>
                    <td className="py-4 px-6">
                      <div>
                        ...
                        <div className="text-[10px] text-slate-400 mt-0.5">
                          {d.phone} • PAN:{" "}
                          <span className="uppercase text-primary font-bold">{d.pan}</span>
                        </div>
                      </div>
                    </td>
                      ...
                      <div className="flex items-center justify-end gap-1.5">
                        ...
                        <button
                          onClick={() => {
                            if (d.status === "Success") {
                              window.open(
                                `${import.meta.env.VITE_API_URL || "http://localhost:5000/api"}/payments/receipt/${d.id}`,
                                "_blank",
                              );
                            } else {
                              toast.error(
            ...
            <div className="p-6 space-y-4 text-xs font-semibold overflow-y-auto flex-1">
              ...
              {selectedDonation.lastError && (
                <div className="bg-rose-50 border border-rose-100 rounded-xl p-3 flex items-start gap-2 text-rose-600">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-[10px] font-medium leading-relaxed font-mono">
                      {selectedDonation.lastError}
                    </p>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <span className="text-slate-900 text-xs select-all truncate block">
                ...
                <div>
                  ...
                  <span className="text-slate-900 text-xs select-all block">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <span className="text-slate-900 font-mono text-sm uppercase tracking-wider font-bold text-[#7A9D1C] select-all">
                    {selectedDonation.pan || "N/A"}
                  </span>
                </div>
                <div>
                  <span className="text-[10px] text-slate-400 uppercase tracking-wider block">
              ...
              <div className="border-t border-slate-100 pt-3">
                ...
                <div className="flex items-center gap-1.5 font-mono text-[9px] text-slate-500 overflow-x-auto pb-1">
                  ...
                  <span
                    className={`px-2 py-0.5 rounded-md ${selectedDonation.rawStatus !== "INITIATED" ? "bg-slate-100" : "bg-primary/10 text-primary font-bold"}`}
                  >
                    Checkout
                  </span>
                  <ArrowRight className="h-3 w-3 text-slate-300" />
                  <span
                    className={`px-2 py-0.5 rounded-md ${["CHARGED", "PAYMENT_VERIFIED", "DONATION_SAVED", "EMAIL_SENT", "ADMIN_NOTIFIED", "COMPLETED"].includes(selectedDonation.rawStatus) ? "bg-slate-100" : "bg-primary/10 text-primary font-bold"}`}
                  >
                    Charged
                  </span>
                  <ArrowRight className="h-3 w-3 text-slate-300" />
                  <span
                    className={`px-2 py-0.5 rounded-md ${selectedDonation.status === "Success" ? "bg-emerald-50 text-emerald-600 font-bold" : selectedDonation.status === "Refunded" ? "bg-purple-50 text-purple-600 font-bold" : selectedDonation.status === "Failed" ? "bg-rose-50 text-rose-600 font-bold" : "bg-slate-100"}`}
                  >
              ...
              {selectedDonation.status === "Success" && (
                <>
                  <button
                    onClick={() => {
                      window.open(
                        `${import.meta.env.VITE_API_URL || "http://localhost:5000/api"}/payments/receipt/${selectedDonation.id}`,
                        "_blank",
                      );
                    }}
                  ...
                  <button
                    onClick={async () => {
                      ...
                      if (
                        window.confirm(
                          `Are you sure you want to issue a refund of ₹${selectedDonation.amount.toLocaleString("en-IN")} for ${selectedDonation.donor}?`,
                        )
                      ) {
